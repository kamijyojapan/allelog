...
### 追加
- **即時PDF作成（ハイブリッド方式）**: doPost内で即座にPDF作成を試み、成功すれば数秒でレポートが完成
- **詳細なエラーログ**: PDF作成失敗時の原因を特定するため、各ステップの実行時間とエラー詳細を記録
- **フォールバック処理**: 即時作成が失敗した場合は自動的にキューに保存し、1分以内にトリガーで処理

### 改善
- **処理時間の表示**: 即時作成成功時は実際の処理時間（秒数）をユーザーに表示
- **失敗理由の通知**: 即時作成失敗時はその理由をユーザーに通知

### 技術詳細
- **GAS実行時間制限**: doPostは30秒、トリガーは6分の制限があるため、軽量なデータは即座に、重いデータはキュー経由で処理
- **ログポイント**: フォルダ取得、テンプレートコピー、ドキュメント開く、ヘッダー処理、テーブル処理、画像処理、PDF変換、ファイル作成の各ステップで時間を計測
