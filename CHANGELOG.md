...
### 追加
- **ローカルサーバー機能**: PCで `npm start` を実行すると、スマホから直接レポートをアップロードできるサーバーが起動
- **📡 サーバー送信ボタン**: ドクターモードに新設。同一Wi-Fiネットワーク内のPCに直接送信可能
- **サーバーIP設定**: 設定画面でPCのIPアドレスを保存・管理
- **自動ポーリング機能**: manager.htmlが5秒ごとにサーバーをチェックし、新着レポートを自動表示
- **Node.js依存関係**: `package.json` と `server.js` を追加（Express.js使用）

### 改善
- **2つの送信方法を提供**:
  - 📡 サーバー送信（同一Wi-Fi、即座に反映）
  - 📨 Web Share送信（Bluetooth・AirDrop・Nearby Share）
- **README.mdにサーバーモード手順を追加**: セットアップから使用方法まで詳細に記載
